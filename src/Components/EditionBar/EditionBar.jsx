...
class EditionBar extends React.Component {
  ...
  render() {
    ...
    return (
      ...
      <Link to={'/infoNote'}>
        <button disabled = {this.props.displayEvent ? false : true} className="editionBar__button buttons" onClick={this.props.showInfoNote}><img src={require('../../images/information.-button.svg')} className="navBar__images" /><span className={"editionBar__span buttonFont " + (this.props.displayEvent ? 'showControl' : 'hiddenControl')}><strong>Info</strong> <br /> {'"'+this.props.noteTitleSelected+'"'} </span></button>
       </Link>
